...
- Browser code cannot write local Markdown files by itself. Keep direct palette saving behind local Vite dev middleware, validate the target path stays inside `palettes/`, and treat duplicate-save `409` console errors as expected during negative-path QA.
